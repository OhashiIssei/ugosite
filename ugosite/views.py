...
def index(request):
    ...
    context = {
        ...
        'module_list' : Module.objects.filter(parent_module__isnull=True)
    }
...
class GenreListView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        genre = Genre.objects.filter(parent_genre__isnull = True).first()
        # genre = Genre.objects.get(name="数学I")
...
class ModuleDetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["module_list"] = [context["module"]]
        return context
...
def transform_to_low_and_up(sentence,startbr,endbr):
    m = re.search("\\\\dint",sentence)
    if m:
        target = sentence[m.end():]
        # target = transform_to_low_and_up(sentence[m.end():],startbr,endbr)
        r = read_invaiants(target,startbr,endbr)
        low = r[0]
        high = r[1]
        sentence = sentence.replace("\\dint"+low+high,"\\displaystyle\\int_"+low+"^"+high)
        sentence = transform_to_low_and_up(sentence,startbr,endbr)
        print(target[:20].replace(low+high,"\\displaystyle\\int_"+low+"^"+high))
    return sentence

def read_invaiants(sentence,startbr,endbr):
    depth = 0
    start = 0
    end = 0
    matchs = []
    for i,c in enumerate(sentence):
        if c == startbr:
            ...
            if depth == 0:
                ...
                matchs.append(sentence[start:end+1])
        elif depth == 0:
                matchs.append(sentence[i])
        if len(matchs)>1:
            break
    return matchs
...
def make_slug(name):
    en_name = tr.translate(name, src="ja", dest="en").text
...
def add_problem_from_tex(module,path):
    article_name = os.path.basename(path).replace(".tex","")
    Article.objects.filter(name =article_name ).delete()
    ...
    for question in questions[1:]:
        ...
        text = transform_to_low_and_up(text,"{","}")
        text = text.replace("\\vv","\\overrightarrow").replace("\\bekutoru","\\overrightarrow").replace("\\doo","^{\\circ}").replace("\\C","^{\\texc{C}}")
        text = text.replace("\\PP","^{\\texc{P}}").replace("\\QQ","^{\\texc{Q}}").replace("\\RR","^{\\texc{R}}")
        ...
        text = re.sub("%+[^\n]*\n","\n",text)
        text = text.replace("\\beda","<ol>").replace("\\eeda","</ol>").replace("\\benu","<ol>").replace("\\eenu","</ol>").replace("\\begin{itemize}","<ol>").replace("\\end{itemize}","</ol>").replace("\\begin{enumerate}","<ol>").replace("\\end{enumerate}","</ol>")
        ...
        text = text.replace("\\begin{mawarikomi}{}{","").replace("\n}\n","\n").replace("\\end{mawarikomi}","")

        text = text.replace("\\vfill","").replace("\\hfill","")
        text = text.replace("\\ifkaisetu","")
...
def make_module(parent_module,name):
    modules = Module.objects.filter(name = name)
    if modules:
        for article in modules[0].article_set.all():
...
def add_problem_from_texs(parent_module,folder_path):
    folderName = os.path.basename(folder_path)
    module = make_module(parent_module,folderName)
    for fileName in os.listdir(folder_path):
        if not fileName.startswith('.') and not "阪大" in fileName:
            file_path = "%s/%s" % (folder_path,fileName)
            if fileName.endswith(".tex"):
                # print(fileName)
                article_name = fileName.replace(".tex","")
                print(article_name)
                add_problem_from_tex(module,file_path)
            elif os.path.isdir(file_path):
                print(fileName)
                add_problem_from_texs(module,file_path)
        

# add_problem_from_texs(None,"/Users/greenman/Library/Mobile Documents/com~apple~CloudDocs/講師関係/2021 西京 17期3年")

                
# for module in Module.objects.filter(name__contains = "Documents"):
#     print(module)
...
# for problem in Problem.objects.filter(text__contains = "軸で囲まれた図形の面積 $S$ を $k$ を用いて表せ．ただし，"):
#     print(problem.article_set.first())
...
class FolderListView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        folders = Folder.objects.filter(folders__isnull = True)
...
def add_icons(folderPath):
    for fileName in os.listdir(folderPath):
        add_icon("%s/%s" % (folderPath,fileName),fileName)

# add_icons("/Users/greenman/Desktop/web-projects/django_projects/ugosuu/media_local/icons")

        
# for genre in Genre.objects.all():
#     genre.name = join_diacritic(genre.name)
#     genre.save()

def genre_from_problems_name():
    for problem in Problem.objects.exclude(name__contains = "年度"):
        name = problem.name
        name = name.replace("　"," ")
        name = re.sub("\s整数$"," 整数の性質",name)
        name = re.sub("\s平面図形とベクトル$"," ベクトルと平面図形",name)
        name = re.sub("\s指数対数関数$"," 指数関数と対数関数",name)
        name = re.sub("\s等差数列と等比数列$"," 数列とその和",name)
        name = re.sub("\sいろいろな数列$"," いろいろな数列の和",name)
        name = re.sub("\s数学I２次関数$"," 数学I ２次関数",name)
        name = re.sub("\s2次関数$"," ２次関数",name)
        name = re.sub("\s2\s次関数$"," ２次関数",name)
        name = re.sub("\s式と証$"," 式と証明",name)
        name = re.sub("\s二項係数$"," 二項定理",name)
        name = re.sub("\s空間ベクトル$"," 空間のベクトル",name)
        name = re.sub("\s平面ベクトル$"," 平面上のベクトル",name)
        name = re.sub("\s数学Ⅲ"," 数学III",name)
        name = re.sub("\s漸化式"," 漸化式の数列",name)
        name = re.sub("\s三角比"," 図形と計量",name)
        
        subject = Genre.objects.get(name="高校数学")
        for genre in Genre.objects.filter(type="SUB"):
            if re.search("\s%s\s" % genre.name,name):
                name = re.sub("\s%s\s" % genre.name," ",name)
                subject=genre
        
        genres=[]
        for genre in subject.descendants():
            if re.search("\s%s$" % genre.name,name):
                genres.append(genre) 
        
        if genres:
            mindepth=len(genres[0].ancestors())
            maingenre = genres[0]
            for genre in genres[1:]:
                depth = len(genre.ancestors())
                if depth<mindepth:
                    mindepth = depth
                    maingenre = genre
            name = re.sub("\s%s$" % maingenre.name,"",name)
            # print(name)
            print(genres)
            print(maingenre)
            problem.name = name
            problem.save()
            problem.genres.clear()
            problem.genres.add(maingenre)

# genre_from_problems_name()
